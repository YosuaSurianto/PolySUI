module polysui::market {
    ...
    public fun create_market(
        ...
        let mut votes = vector::empty<vector<address>>();
        let mut i = 0;
        while (i < options_count) {
            vector::push_back(&mut votes, vector::empty<address>());
            i = i + 1;
        };
    ...
    public fun vote(
        ...
        assert!(value >= market.fee, EInsufficientPayment);

        let coin_balance = coin::into_balance(payment);
        balance::join(&mut market.balance, coin_balance);
        ...
        // Record vote
        let option_voters = vector::borrow_mut(&mut market.votes, option_index);
        vector::push_back(option_voters, voter);

        // global anti double vote
        vector::push_back(&mut market.voters, voter);

        market.total_votes = market.total_votes + 1;
    ...
    public fun get_winner(market: &VotingMarket): u64 {
        ...
        while (i < len) {
            let option_voters = vector::borrow(&market.votes, i);
            let votes = vector::length(option_voters);
    ...
    fun assert_unique_strings(values: &vector<String>) {
    ...
    fun get_vote_counts(votes: &vector<vector<address>>): vector<u64> {
        let mut counts = vector::empty<u64>();
        let len = vector::length(votes);
        let mut i = 0;

        while (i < len) {
            let option_voters = vector::borrow(votes, i);
            vector::push_back(&mut counts, vector::length(option_voters));
            i = i + 1;
        };
    ...
    public fun count_total_votes(market: &VotingMarket): u64 {
        market.total_votes
    }

    public fun count_total_fees(market: &VotingMarket): u64 {
